/* =========================================================
   明月證券 v4.3.4
   Plugin Data Adapter - Google UID Bootstrap
   ========================================================= */
...
const CACHE_KEYS = {
    users: "mingyue_user_v43",
    stocks: "mingyue_stocks_v43",
    companies: "mingyue_companies_v43",
    news: "mingyue_news_v43",
    portfolios: "mingyue_portfolio_v43",
    transactions: "mingyue_transactions_v43",
    historyData: "mingyue_history_v43"
};

function validKey(key) { return PATHS.includes(String(key)); }
function getCurrentUid() {
    return auth.currentUser?.uid || window.MingyueAuth?.user?.uid || localStorage.getItem("mingyue_current_google_uid") || null;
}
...
async function externalRead(key) {
    ...
    } catch (e) { console.warn("外掛讀取失敗", key, e); }
    return undefined;
}
async function externalWrite(key, value) {
    ...
    } catch (e) { console.warn("外掛寫入失敗", key, e); }
    return false;
}
async function externalReadAll() {
    ...
    } catch (e) { console.warn("外掛全資料讀取失敗", e); }
    return undefined;
}
async function externalWriteMany(data) {
    ...
    if (!p) return false;
    try {
        if (typeof p.writeMany === "function") { await p.writeMany(data || {}); return true; }
    } catch (e) { console.warn("外掛批次寫入失敗", e); }
    return false;
}
function filterPaths(data) {
    const result = {};
    for (const key of PATHS) if (Object.prototype.hasOwnProperty.call(data || {}, key)) result[key] = data[key];
    return result;
}
function cacheData(data) {
    try {
        ...
        for (const key of PATHS) {
            ...
            if (["users", "portfolios", "transactions"].includes(key)) {
                const account = uid ? value?.[uid] : null;
                if (account !== undefined && account !== null) localStorage.setItem(cacheKey, JSON.stringify(account));
            } else localStorage.setItem(cacheKey, JSON.stringify(value));
        }
    } catch (e) { console.warn("LocalStorage fallback 寫入失敗", e); }
}

async function bootstrapLegacyV42(user) {
    ...
    } catch (e) { console.warn("Google UID 帳號預載入失敗", e); }

    const existing = account && typeof account === "object" ? account : {};
    const balance = Number.isFinite(Number(existing.balance)) ? Number(existing.balance) : 0;
    const legacyUser = {
        ...existing,
        accountId: uid,
        googleUid: uid,
        name: existing.name || user.displayName || user.email || "Google 使用者",
    ...
    localStorage.setItem("mingyue_user_v42", JSON.stringify(legacyUser));

    if (!account) {
        await update(ref(db), {
            [`users/${uid}`]: { ...legacyUser, createdAt: Date.now(), lastLoginAt: Date.now() },
            [`portfolios/${uid}`]: {},
            [`transactions/${uid}`]: []
        });
    }
...
async function read(key) {
    ...
    } catch (e) { console.warn("Firebase 讀取失敗", key, e); return null; }
}
async function write(key, value) {
    if (!validKey(key)) throw new Error("不允許的資料路徑：" + key);
    const externalOK = await externalWrite(key, value);
    try { await set(ref(db, key), value); }
    catch (e) { if (!externalOK) throw e; console.warn("Firebase 鏡像失敗", key, e); }
    return true;
}
async function writeMany(data) {
    ...
    catch (e) { if (!externalOK) throw e; console.warn("Firebase 批次鏡像失敗", e); }
...
async function readAll() {
    ...
    try {
        ...
        cacheData(data);
        return data;
    } catch (e) { console.warn("Firebase 全資料讀取失敗", e); return {}; }
}
async function preload() {
    const data = await readAll();
    console.log(data && Object.keys(data).length ? "明月證券 v4.3.4：資料已預載入" : "明月證券 v4.3.4：沒有遠端資料");
    return data;
}

window.MingyueDataPlugin = Object.freeze({ version: "4.3.4", paths: Object.freeze([...PATHS]), read, write, writeMany, readAll, preload, getCurrentUid, isReady: true });
window.MingyueDataAdapter = window.MingyueDataPlugin;
window.MINGYUE_V433 = true;

await setPersistence(auth, browserLocalPersistence).catch(e => console.warn("Firebase Auth 持久化設定失敗", e));

// 關鍵：先取得 Firebase currentUser，再讓舊 v4.2 script.js 讀取 v42 cache。
await new Promise(resolve => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async user => {
        try {
            if (user) {
                await bootstrapLegacyV42(user);
                window.MINGYUE_CURRENT_UID = user.uid;
                window.MINGYUE_ACCOUNT_ID = user.uid;
            }
        } catch (e) { console.warn("Google UID bootstrap 失敗", e); }
        finally {
            unsubscribe();
            finish();
        }
...
console.log("明月證券 v4.3.4 Plugin Data Adapter 已載入");
await preload();
try {
    await import("./google-auth.js?v=4.3.4");
    window.MINGYUE_V43 = true;
    console.log("明月證券 v4.3.4 Google Account 模組已接入");
} catch (e) {
    window.MINGYUE_V43 = false;
    console.warn("Google Account 模組載入失敗", e);
}
